...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _loadCompletedChallenges() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _completedChallengesCache = logsSnap.docs
  ...
  Future<void> _checkDailyReset() async {
    ...
    try {
      ...
      debugPrint("Error reset diario: $e");
      await _loadRandomDailyChallenges();
    }
  }
  ...
  void _setupDailyTimer({required DateTime lastResetTime}) {
    ...
    _dailyResetTimer = Timer.periodic(const Duration(seconds: 1), (_) {
      if (mounted) {
        setState(() {
          _timeUntilReset = resetTime.difference(DateTime.now());
          if (_timeUntilReset.isNegative) _checkDailyReset();
        });
      }
    });
  }
  ...
  Future<void> _loadRandomDailyChallenges() async {
    ...
    try {
      ...
      final disponibles = challengesSnap.docs.where((doc) => !completedIds.contains(doc.id)).toList();
      disponibles.shuffle();
  ...
  Future<void> _finalizarActividad(String id, String titulo, int premio) async {
    ...
    setState(() {
      _completedChallengesCache.insert(0, {'titulo': titulo, 'recompensa': premio, 'id_reto_completado': id});
    ...
    try {
      ...
      if (nuevoNivel != nivel) setState(() => nivel = nuevoNivel);
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // --- BARRA DE NAVEGACIÓN IMPLEMENTADA ---
      bottomNavigationBar: BottomNavigationBar(
        backgroundColor: const Color(0xFF1A1A1A),
        selectedItemColor: Colors.orange,
        unselectedItemColor: Colors.white54,
        currentIndex: 0, // Posición Home
  ...
  Widget _buildCompletedChallengesList() {
    if (_completedChallengesCache.isEmpty) {
      return const Text("No hay retos hoy", style: TextStyle(color: Colors.white54));
    }
    final lista = _mostrarTodosLosLogros ? _completedChallengesCache : _completedChallengesCache.take(3).toList();
    return Column(
  ...
  Widget _buildDailyChallengesList() {
    ...
    if (_dailyChallenges.isEmpty) return const Text("¡Completado!", style: TextStyle(color: Colors.white54));

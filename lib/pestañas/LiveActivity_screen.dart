...
class _LiveActivityScreenState extends State<LiveActivityScreen>
  ...
  // ── Para volver al HUD original: cambia _buildHUDTactico → _buildHUDClasico
  //    en _buildHUD() y _buildHUDMiniTactico → _buildHUDMiniClasico abajo.
  Widget _buildHUD() {
    if (!isTracking) return const SizedBox.shrink();
    if (_hudMinimizado && !isPaused) return _buildHUDMiniTactico();
    return _buildHUDTactico();
  }

  // ── HUD TÁCTICO ──────────────────────────────────────────────────────────────
  Widget _buildHUDTactico() {
    // Stat extra contextual (META / RUTA / ZONA / ZONAS)
    String? extraLabel;
    String? extraValor;
    Color   extraColor = _kWaterLight;
    if (_objetivoGlobal != null) {
      extraLabel = 'META';
      extraValor = '${(_progresoGlobal * 100).toInt()}%';
      extraColor = _globalConquistado ? _kVerde : _p.globalRed;
    } else if (_rutaGuiada != null) {
      extraLabel = 'RUTA';
      extraValor = '${(_porcentajeRuta * 100).toInt()}%';
      extraColor = _rutaCompletada ? _kVerde : _kWaterLight;
    } else if (_modoSolitario) {
      extraLabel = 'ZONA';
      extraValor = _barrioActual != null
          ? '${(_barrioActual!.porcentajeCubierto * 100).toInt()}%'
          : '--';
      extraColor = _kVerde;
    } else if (!_modoRuta) {
      extraLabel = 'ZONAS';
      extraValor = '${_territoriosVisitadosEnSesion.length}';
      extraColor = _kWaterLight;
    }
        ...
        child: AnimatedBuilder(
          ...
          builder: (_, child) => Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(6),
              border: Border.all(
                  color: Colors.white.withValues(alpha: 0.08 + _pulso.value * 0.06)),
              boxShadow: [
                BoxShadow(
                    color: AppColors.red.withValues(alpha: 0.12 + _pulso.value * 0.08),
                    blurRadius: 16),
              ],
            ),
            child: child,
          ...
          child: IntrinsicHeight(
            child: Row(
              ...
              children: [
                // Barra roja lateral — identidad táctica
                Container(
                  width: 3,
                  decoration: BoxDecoration(
                    color: AppColors.red,
                ...
                Expanded(
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        // Fila 1: distancia (dato primario, tamaño dominante)
                        Row(
                          crossAxisAlignment: CrossAxisAlignment.baseline,
                          textBaseline: TextBaseline.alphabetic,
                          children: [
                            Text(
                              _distanciaTotal.toStringAsFixed(2),
                              style: GoogleFonts.rajdhani(
                                fontSize: 46,
                                fontWeight: FontWeight.w700,
                                color: _kGold,
                        ...
                        const SizedBox(height: 6),
                        // Fila 2: ritmo + tiempo + extra
                        Row(children: [
                          _tacStat('MIN/KM', _ritmoStr, _kWaterLight),
                          _tacDivider(),
                          _tacStatTimer(),
                          if (extraLabel != null) ...[
                            _tacDivider(),
                            _tacStat(extraLabel, extraValor!, extraColor),
                          ],
                          if (_boostXpActivo) ...[
                            _tacDivider(),
                            _tacStat('BOOST', '×2', _kGoldLight),
                          ],
                        ]),
                      ],
                    ),
  ...
  Widget _tacStat(String label, String valor, Color color) =>
      Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start,
          children: [
        Text(label,
            style: GoogleFonts.rajdhani(
                fontSize: 8, fontWeight: FontWeight.w600,
                color: Colors.white.withValues(alpha: 0.40), letterSpacing: 1.2)),
        Text(valor,
            style: GoogleFonts.rajdhani(
  ...
  Widget _tacDivider() => Container(
      margin: const EdgeInsets.symmetric(horizontal: 10),
      width: 1, height: 28,
      color: Colors.white.withValues(alpha: 0.12));

  Widget _tacStatTimer() => CustomTimer(
        controller: _timerController,
        builder: (_, remaining) {
          final str = isTracking
              ? '${remaining.hours}:${remaining.minutes.toString().padLeft(2, '0')}:${remaining.seconds.toString().padLeft(2, '0')}'
              : '--:--';
          return _tacStat('TIEMPO', str, isPaused ? _p.goldDim : Colors.white.withValues(alpha: 0.70));
        },
      );

  // ── HUD MINI TÁCTICO (minimizado) ───────────────────────────────────────────
  Widget _buildHUDMiniTactico() => Padding(
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              ...
              Text(' KM',
                  style: GoogleFonts.rajdhani(
                      ...
                      fontWeight: FontWeight.w600, fontSize: 11, letterSpacing: 1)),
              Container(
                  margin: const EdgeInsets.symmetric(horizontal: 10),
                  width: 1, height: 14,
              ...
              Text(
                _ritmoStr,
                style: GoogleFonts.rajdhani(
                    color: _kWaterLight, fontWeight: FontWeight.w700,
                    fontSize: 16, letterSpacing: 0.5,
                    fontFeatures: const [FontFeature.tabularFigures()]),
              ),
              Text(' /KM',
              ...
              Icon(CupertinoIcons.chevron_up,
                  color: Colors.white.withValues(alpha: 0.30), size: 12),
            ]),
          ),
        ),
      );

  // ── HUD CLÁSICO (preservado — para revertir cambia _buildHUDTactico por _buildHUDClasico) ──
